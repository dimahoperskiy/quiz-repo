...
import {User} from '../models/User'; // модель MongoDB
...
router.post('/login', async (req, res) => {
    ...
    try {
        const user = await User.findOne({ email }) as { _id: string; email: string; passwordHash: string };
...
router.get('/me', async (req, res) => {
    ...
    try {
        ...
        const user = await User.findById(payload.userId) as { _id: string; email: string; passwordHash: string };
        if (!user) {
